...
const UUID = process.env.UUID || '6e7f1b57-1957-4e42-a24c-83f87f81ad0d'; // 运行哪吒v1,在不同的平台需要改UUID,否则会被覆盖
...
function handleVlessConnection(ws, msg) {
  try {
    const [VERSION] = msg;
    // 检查 msg 长度是否足够包含 UUID
    if (msg.length < 17) return false;
    const id = msg.slice(1, 17);
    if (!id.every((v, i) => v == parseInt(uuid.substr(i * 2, 2), 16))) return false;
    
    // 检查 msg 长度是否足够包含 optLength
    if (msg.length < 18) return false;
    const optLength = msg.slice(17, 18).readUInt8();
    
    // 计算后续数据的起始位置
    // 1(ver) + 16(id) + 1(optLen) + optLength + 1(cmd)
    let i = 19 + optLength; 
    
    // 检查是否有足够的字节读取端口 (2 bytes)
    if (msg.length < i + 2) return false;
    const port = msg.slice(i, i += 2).readUInt16BE(0);
    
    // 检查是否有足够的字节读取 ATYP (1 byte)
    ...
    const connectAndPipe = (addr, p) => {
        net.connect({ host: addr, port: p }, function() {
            this.write(msg.slice(i));
            duplex.on('error', () => {}).pipe(this).on('error', () => {}).pipe(duplex);
        }).on('error', () => {});
    };
    resolveHost(host)
      .then(resolvedIP => connectAndPipe(resolvedIP, port))
      .catch(error => connectAndPipe(host, port));
